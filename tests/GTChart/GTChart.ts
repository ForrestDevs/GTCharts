...
export default class ChartImp implements GTChart {
  id: string
  ...
  private _candlePane: Nullable<CandlePane> = null
  private _xAxisPane: Nullable<XAxisPane> = null

  constructor (container: HTMLElement, options?: Options, id?: string) {
    this.id = id ?? createId('k_line_chart')
    this._container = container
    this._chartContainer = this._initContainer(container)
  ...
  private _initContainer (container: HTMLElement): HTMLElement {
    const chartContainer = createDom('div', {
      position: 'relative',
      width: '100%',
      outline: 'none',
      borderStyle: 'none',
  ...
  private _createPane<P extends DrawPane> (
    DrawPaneClass: new (rootContainer: HTMLElement, afterElement: Nullable<HTMLElement>, chart: GTChart, id: string) => P,
    id: string,
    options?: PaneOptions
  ): P {
  ...
  getCandlePane (): Nullable<CandlePane> { return this._candlePane}

  getXAxisPane (): Nullable<XAxisPane> { return this._xAxisPane }
  ...
  crosshairChange (crosshair: Crosshair): void {
    ...
    if (actionStore.has(ActionType.OnCrosshairChange)) {
      const indicatorData: Record<string, Record<string, any>> = {}
      this._drawPanes.forEach(pane => {
        const id = pane.getId()
        const paneIndicatorData: Record<string, any> = {}
